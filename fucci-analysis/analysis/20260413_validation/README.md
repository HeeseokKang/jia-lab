...
- `corrected/` (Phase 1, **gitignored, ~18 GB**) — flat-field-corrected 647
  TIFFs from the (now removed) t0-reference correction. NOT used by the pipeline
  (see Phase 2 finding below); the flat-field scripts were deleted 2026-06-10.
...
| --- | --- | --- |
| `corrected/` | ~18 GB | regen script removed 2026-06-10 (see git history); approach rejected |
...
## Note — 2026-06-10 cleanup

The three Phase-1 flat-field scripts (`quantify_background.py`,
`plot_background_gradient.py`, `flat_field_correction.py`) were **removed** — the
flat-field approach was rejected (it suppressed FUCCI signal; the variant pipeline
uses per-cell local background instead). The CSV records below are kept; the
scripts remain in git history if ever needed.

## Note — 2026-06-01 cleanup

QC **PNGs** in this folder and `plots/` were removed from git (regeneratable
from the scripts), along with the `corrected/` (~18 GB) and
`segmentation_test/masks/` data on disk. The **CSV tables** and the findings
below are retained. The one-off `segment_test.py` / `extract_intensity.py` and
the `nuclear_intensity_raw_comparison.py` diagnostic script were deleted; the
`R1_1_nuclear_647_comparison.csv` they produced is kept as the record of the
647-source finding. Recreate masks with `timeseries_one_well.py` (the corrected
stacks are not regeneratable — the flat-field script was removed 2026-06-10).

## `segmentation_test/` inventory (Phase 2)

All outputs are for well `R1_1`. The pipeline scripts that produced each file
are noted in parentheses. (PNGs listed below are regeneratable; only CSVs remain
tracked.)
...
### `masks/` (gitignored, regeneratable)
...
`tracking_one_well.py` and `nuclear_intensity_one_well.py`.
...
biological signal that gets normalized away. Source CSV (retained):
`R1_1_nuclear_647_comparison.csv`. (The diagnostic script and its PNG were
removed in the 2026-06-01 cleanup.)

**Decision**: this pipeline uses **raw 647**. The 647 row-axis gradient is
preserved, but biological FUCCI signal is not destroyed. This finding is what led
the later variant pipeline to adopt **per-cell local background subtraction**
instead of any flat-field correction (the flat-field scripts were removed).
